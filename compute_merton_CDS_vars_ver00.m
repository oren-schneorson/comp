...
precision = 1e-10; % precision of iterative process

r=.5;

%{
procedure: two loops, inner and outer
outer loop: compute E given sigma
inner loop: get sigma, given DD(PoD) and V
...
try
    ...
    while diff_sigma_V > precision
        ...
        ln_V_F = T.DD.*sigma_V0-mu_V+.5*sigma_V0.^2;
        V1 = T.F .* exp(ln_V_F);

        % compute new sigma_V1 from V1
        dV1 = log(V1(2:end)./V1(1:end-1));
        idx = dV1 < prctile(dV1,99) & dV1 > prctile(dV1,1);
        dV1(~idx) = NaN;
        
        % compute sigma_V1 based on V1
        
        %sigma_V1 = std(dV1, 'omitnan'); % unconditional volatility
        % conditional volatility
        sigma_V1 = arrayfun(@(datadate) std(dV1(...
            T.datadate < datadate &...
            T.datadate >= datadate-365), 'omitnan'), T.datadate);
        
        sigma_V1 = sigma_V1*sqrt(252); % annalize
        diff_sigma_V = abs(sigma_V1-sigma_V0); % iterate until
    ...
    if counter_inner > 50
        ME = 'algorithm did not converge (outer)';
        T = table;
        return
    end
    ...
    % TODO: V0 needs to be the intial condition
    E0 = E(V0, T.F, T.r, 1,...
        d_1(V0, T.F, T.r, 1, sigma_V0),...
        d_2(V0, T.F, T.r, 1, sigma_V0));
    E1 = E(V1, T.F, T.r, 1,...
        d_1(V1, T.F, T.r, 1, sigma_V1),...
        d_2(V1, T.F, T.r, 1, sigma_V1));
    diff_E = sum(abs(E1-E0)); % iterate until
    ...
    vars_to_keep = {'datadate', 'mcap', 'E', 'F', 'V', 'mu_V', 'sigma_V', 'DD'};
    T = T(:, vars_to_keep);
catch ME
   rethrow(ME) 
   T = table; 
