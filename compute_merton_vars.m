...
% Black-Scholes-Merton Formula, for a given date
BSM = @(V_,sigma_V_, t) E(...
    V_, T.F(t), T.r(t), 1, d_1(V_, T.F(t), T.r(t), 1, sigma_V_), d_2(V_, T.F(t), T.r(t), 1, sigma_V_)...
    ) - T.mcap(t);
...
try
    ...
    while diff_sigma_V > precision
        ...
        Censoring these changes when computing sigma_V takes care of
        this.
        ...
        sigma_V_0 = sigma_V_0*sqrt(252); % annualize (assuming daily freq)

        % for each t, solve equation (2) in Vassalou and Xing (2004) 
        % for V_, given sigma_V_0
        for t = 252:size(T,1)
            %
            t
            V0(t)
            ...
            [V1(t), fval(t)] = my_NR([], @(V_) BSM(V_, sigma_V_0(t), t), V0(t));
            % requires optimization toolbox
        ...
        title('Total difference between iterations')

        subplot(2,2,3)
        hold on
        plot(T.datadate(2:end), V0(2:end)./V0(1:end-1)-1)
        plot(T.datadate(2:end), V1(2:end)./V1(1:end-1)-1)
        title('Iteration: \sigma_V')
        legend({'Initial condition', 'Output'})
